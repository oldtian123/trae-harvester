...
exports.listRunningContainers = listRunningContainers;
exports.matchContainerByModelOrder = matchContainerByModelOrder;
...
function listRunningContainers() {
    return new Promise((resolve, reject) => {
        const child = (0, child_process_1.spawn)('docker', ['ps', '--format', '{{.Names}}'], { shell: false });
        let stdout = '';
        let stderr = '';
 ...
 * @throws            0 个或多个命中时抛错
 */
function matchContainerByModelOrder(containers, modelOrder) {
    ...
    if (matched.length === 0) {
        throw new Error(`未找到模型顺序 m${String(modelOrder).padStart(2, '0')} 对应的运行中容器。` +
            `该轮次容器可能尚未创建或已被销毁。当前运行容器: [${containers.join(', ') || '无'}]`);
    }
    if (matched.length > 1) {
        throw new Error(`匹配到多个模型顺序为 m${String(modelOrder).padStart(2, '0')} 的容器: [${matched.join(', ')}]，无法确定目标。`);
    }
    return matched[0];
}
/**
...
function restartContainer(container) {
    return new Promise((resolve, reject) => {
        const child = (0, child_process_1.spawn)('docker', ['restart', container], { shell: false });
        let stderr = '';
        child.stderr?.on('data', d => stderr += d.toString());
        child.on('error', (err) => {
 ...
 */
function parseShell(shellConfig) {
 ...
 */
function execInContainer(container, command, workdir, shellConfig, timeoutMs = 0) {
...
//# sourceMappingURL=dockerExec.js.map