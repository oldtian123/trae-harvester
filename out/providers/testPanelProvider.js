...
class TestPanelProvider {
    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                case 'toggleMcp':
                    // 控制窗口侧 HTTP server
                    if ((0, windowServer_1.isWindowServerRunning)()) {
                        (0, windowServer_1.stopWindowServer)();
                        vscode.window.showInformationMessage('🔴 MCP Server 已停止');
                    }
                    else {
                        const port = await (0, windowServer_1.startWindowServer)(this._extensionContext);
                        if (port) {
                ...
                case 'updateIdentifiers':
                    ...
                    updatePlanIdentifiers(message.repoId, message.branch, message.modelId, message.promptId);
                    break;
                case 'gitFetch':
     ...
     */
    async _syncCurrentState() {
        const plan = (0, testRunner_1.getCurrentPlan)();
        const isMcpRunning = (0, windowServer_1.isWindowServerRunning)();
        ...
        const repoOptions = config.get('repoOptions') || [];
        // 获取当前分支
